...
class Dao(object):
    ...
    def update(self, dto_instance):
        object_dict = vars(dto_instance)  # Convert the instance to a dictionary
        id_value = dto_instance.id
        # Filter out 'id' from the dictionary and values
        filtered_dict = {k: v for k, v in object_dict.items() if k != 'id'}
        values = list(filtered_dict.values())
        set_clause = ', '.join([f"{name} = ?" for name in filtered_dict.keys()])

        stmt = f"UPDATE {self._table_name} SET {set_clause} WHERE id = ?"
        # Ensure that values only includes relevant entries
        self._conn.execute(stmt, values + [id_value])
        self._conn.commit()
